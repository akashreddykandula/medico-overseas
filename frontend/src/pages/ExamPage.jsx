...
const EXAM_CONTENT = {
  fmge: {
    title: 'FMGE Exam',
    subtitle: 'The licensing exam required for MBBS graduates from foreign universities to practice medicine in India.',
    sections: [
      {
  ...
  nmat: {
    title: 'NMAT Exam',
    subtitle: 'Understand eligibility, exam pattern, and how we support your preparation and registration.',
    sections: [
      {
...
const ExamPage = () => {
  ...
  return (
    <>
      ...
      <PageHero eyebrow="LICENSING EXAM" title={content.title} subtitle={content.subtitle} />

      <div className="section-container grid grid-cols-1 gap-12 py-16 lg:grid-cols-3">
        <div className="space-y-12 lg:col-span-2">
          {content.sections.map((s) => (
            <section key={s.heading}>
              <h2 className="font-heading text-2xl font-bold text-navy-600">{s.heading}</h2>
        ...
        <aside className="lg:col-span-1">
          <div className="glass-card sticky top-28 !bg-white p-6">
            <EnquiryForm source="exam_page" title={`Get Guidance on ${content.title}`} />
          </div>
        </aside>
      </div>
    </>
